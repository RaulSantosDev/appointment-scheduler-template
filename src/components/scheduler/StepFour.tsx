    ...
    <section className="flex flex-col items-center justify-center min-h-[600px] bg-[#0f0f0f] text-white p-4 font-sans animate-in fade-in duration-500">
      ...
      <div className="flex gap-1.5 mb-10">
        {[1, 2, 3, 4].map((step) => (
          <div 
            key={step} 
            className={`h-1 w-8 rounded-full ${step === 4 ? 'bg-[#c5a048]' : 'bg-[#c5a048]/20'}`} 
          />
        ))}
      </div>
      ...
      <div className="w-full max-w-[340px] bg-[#1a1a1a] border border-[#c5a048]/20 rounded-2xl p-6 mb-8 shadow-2xl">
        <div className="flex flex-col items-center space-y-4">
          ...
          <div className="flex items-center gap-2 text-[#c5a048]">
            <Calendar size={18} />
            <span className="text-base font-medium">{draft.date}</span>
          </div>
      ...
      <button
        onClick={onReset}
        className="w-full max-w-[280px] py-3 border-2 border-[#c5a048] text-[#c5a048] rounded-xl font-bold uppercase tracking-widest text-xs transition-all hover:bg-[#c5a048] hover:text-black active:scale-95"
      >
        Agendar otra cita
      </button>
    </section>
