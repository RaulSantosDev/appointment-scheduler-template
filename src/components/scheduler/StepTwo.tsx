...
export default function StepTwo({
  ...
  useEffect(() => {
    let timer : number;
    // Si no hay slots, iniciamos el contador de 10 segundos
    if (slots.length === 0) {
      timer = setTimeout(() => {
        setShowNoSlotsMessage(true);
      }, 2400); 
    } else {
      // Si aparecen slots antes de los 10 segundos, reseteamos el mensaje
    ...
    <section className="max-w-md mx-auto text-white animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      {/* Título de sección de horarios */}
        ...
        <div className="mb-4">
          ...
          ) : showNoSlotsMessage && (<div className="text-15 tracking-[0.2em] text-zinc-500 font-bold uppercase text-center mb-4 animate-in fade-in slide-in-from-right-4 duration-500">
